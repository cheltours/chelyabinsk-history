...
export default function Map({
    ...
    const mapContainer: RefObject<HTMLDivElement | null> = useRef<HTMLDivElement>(null)
    const context = useContext(AppContext)
    const mapDark: StyleSpecification = mapDarkRaw as unknown as StyleSpecification
    const mapLight: StyleSpecification = mapLightRaw as unknown as StyleSpecification
        ...
        if (appPlaces) {
            for (let i = Number(Object.keys(appPlaces)[0]); i <= Object.keys(appPlaces).length; i++) {
                if (appPlaces[i]) {
                    ...
                    const popupNode: HTMLDivElement = document.createElement('div')
                    const popupRender: Root = createRoot(popupNode)
                    const popup: maplibregl.Popup = new maplibregl.Popup().setDOMContent(popupNode)
                    ...
                    if (appPlaces[i].marker === undefined) appPlaces[i].marker = marker
                }
            }
        }
    }
...
export function setUserMarker({ map, position }: { map: maplibregl.Map | null; position: GeolocationPosition }) {
    if (map) {
