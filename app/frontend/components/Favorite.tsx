        ...
        return (
            <>
                <PlaceScheduleModal
                    ...
                    changeVisible={setScheduleModal}
                />
                        ...
                        {placesEntries.map(([key, item]) => (
                            <article key={key} className="favorite-place">
                                ...
                                <div className="favorite-place-info">
                                    <h2>
                                        {item.title}{' '}
                                        <span className="place-favorite-icon">
                                        ...
                                        {item.popup.schedule === undefined ? (
                                            ...
                                            <nav>
                                                <span className="place-schedule-dropdown">
                                                    <button
                                                        className="place-schedule-current"
                                                        onClick={() => {
                                                            if (screenWidth > 1000) {
                                                                setOpenSchedules((element) => ({
                                                                    ...element,
                                                                    [key]: !element[key],
                                                                }))
                                                                setPlaceTitle('')
                                                        ...
                                                        {hours >= item.popup.schedule[todayIndex].openHours &&
                                                        hours < item.popup.schedule[todayIndex].closeHours
                                                            ? 'Открыто до ' +
                                                              String(
                                                                  item.popup.schedule[todayIndex].closeHours
                                                              ).padStart(2, '0') +
                                                              ':' +
                                                              String(
                                                                  item.popup.schedule[todayIndex].closeMinutes
                                                              ).padStart(2, '0')
                                                            : 'Закрыто до ' +
                                                              String(
                                                                  item.popup.schedule[todayIndex].openHours
                                                              ).padStart(2, '0') +
                                                              ':' +
                                                              String(
                                                                  item.popup.schedule[todayIndex].openMinutes
                                                              ).padStart(2, '0')}
                                                        <span>
                                                    ...
                                                    <div
                                                        className="dropdown-schedule-content"
                                                        id={openSchedules[key] ? 'visible' : ''}>
                                                        {Object.keys(item.popup.schedule).map(([key]) => (
                                                            <p
                                                                ...
                                                                {date[Number(key)]}
                                                                <button>
                                                                    {item.popup.schedule[Number(key)]?.dayoff ? (
                                                                        'Выходной'
                                                                    ) : (
                                                                        <>
                                                                            {String(
                                                                                item.popup.schedule[Number(key)]
                                                                                    ?.openHours
                                                                            ).padStart(2, '0')}
                                                                            :
                                                                            {String(
                                    ...
                                    <span className="place-actions">
                                        ...
                                        {item.hasReport !== undefined && (
                                            <button
                                                ...
                                                onClick={() => setReportView(Number(key) + 1)}>
