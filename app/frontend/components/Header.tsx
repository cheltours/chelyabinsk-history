...
export default function Header({
    ...
    const reportPage: React.ReactNode = <Reports map={map} setMobileMenu={setPlacesMenu} screenWidth={screenWidth} />
    ...
    return (
        <>
            <Settings
                setState={setSettingsVisible}
                getState={settingsVisible}
            ...
            <UserGeolocation
                ...
                errorText={errorText}
                UGvisible={UserGeolocationModal}
                setUG={setUserGeolocationVisible}
            />
                    ...
                    <nav className="actions">
                        {Object.entries(navmenu).map(([key, index]) => (
                            <button
                                ...
                                className={getPage === index.page ? 'active' : ''}>
                                <span>
                                    <FontAwesomeIcon icon={getPage === index.page ? index.icon[0] : index.icon[1]} />
                                </span>
                                <p>{index.title}</p>
                            </button>
                        ))}
...
function getGeolocation({
    ...
    setErrorTitle,
    setErrorText,
    setUserGeolocationVisible,
}: {
    map: MapLibre | null
    setErrorTitle: (error: string) => void
...
function geolocationError({
    ...
    switch (error.code) {
        ...
        case error.POSITION_UNAVAILABLE:
            ...
            setErrorText(
                'Мы пытались, но не смогли поймать спутник. Проверьте, включена ли геолокация на устройстве, и попробуйте еще раз.'
            )
            break
    }
}
