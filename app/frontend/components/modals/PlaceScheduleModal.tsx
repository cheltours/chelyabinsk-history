        ...
        <div className="place-schedule-background" id={!visible ? 'invisible' : ''}>
            <div className="place-schedule-modal">
                <nav className="place-schedule-header">
                    <span className="place-schedule-name">
                        <p>{placeName}</p>
                    </span>
                ...
                <div className="place-schedule-content">
                    {Object.entries(schedule).map(([key, index]) => (
                        <p key={key} className="place-schedule-time" id={todayIndex === Number(key) ? 'active' : ''}>
                            {date[Number(key)]}
                            <span>
                                {String(index.openHours).padStart(2, '0')}:{String(index.openMinutes).padStart(2, '0')}
                                &nbsp;-&nbsp;
